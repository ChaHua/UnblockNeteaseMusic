...
hook.request.before = ctx => {
  ...
  req.url =
    (req.url.startsWith("http://")
      ? ""
      : (req.socket.encrypted ? "https:" : "http:") + "//music.163.com") +
    req.url;
  const url = parse(req.url);
  if (
    [url.hostname, req.headers.host].some(host =>
      hook.target.host.includes(host)
    ) &&
    req.method == "POST" &&
  ...
  if (
    hook.target.host.includes(url.hostname) &&
    url.path.startsWith("/weapi/")
  ) {
...
hook.request.after = ctx => {
  ...
  if (
    netease &&
    hook.target.path.includes(netease.path) &&
    ...
    return request
      ...
      .then(buffer => {
        ...
        const patch = string =>
          string.replace(/([\[|{|:]\s*)(\d{16,})(\s*[\]|}|,])/g, '$1"$2L"$3'); // for js precision
        try {
        ...
        if (
          netease.path.includes("manipulate") &&
          [401, 512].includes(netease.jsonBody.code) &&
          !netease.web
        )
      ...
      .then(() => {
        if ("transfer-encoding" in proxyRes.headers)
...
hook.connect.before = ctx => {
  ...
  if (
    [url.hostname, ctx.req.headers.host].some(host =>
      hook.target.host.includes(host)
    )
  ) {
    if (url.port == 80) {
...
const tryCollect = ctx => {
  const req = ctx.req;
  const netease = ctx.netease;
  let trackId = (netease.param.trackIds instanceof Array
    ? netease.param.trackIds
    : JSON.parse(netease.param.trackIds))[0];
  return request(
    "POST",
    "http://music.163.com/api/playlist/manipulate/tracks",
    req.headers,
    `trackIds=[${trackId},${trackId}]&pid=${netease.param.pid}&op=${
      netease.param.op
    }`
  )
...
const tryMatch = ctx => {
      ...
      return match(item.id)
        ...
        .then(song => {
          ...
          const newer = (base, target) => {
            let difference = Array.from([base, target])
              .map(version =>
                version
                  .split(".")
                  .slice(0, 3)
  ...
  if (!jsonBody.data instanceof Array) {
    ...
    target = netease.web
      ? 0
      : parseInt(
          (netease.param.ids instanceof Array
            ? netease.param.ids
